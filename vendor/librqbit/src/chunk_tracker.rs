...
impl ChunkTracker {
    ...
    /// Return the per-piece have-bitfield as a `Vec<bool>` (one entry per piece).
    /// Exposed so external UIs can render a piece map without touching internals.
    pub fn get_have_pieces_vec(&self) -> Vec<bool> {
        self.have.as_slice().iter().by_vals().collect()
    }

    /// Return the per-piece selected-bitfield as a `Vec<bool>` (one entry per piece).
    pub fn get_selected_pieces_vec(&self) -> Vec<bool> {
        self.selected.iter().by_vals().collect()
    }
